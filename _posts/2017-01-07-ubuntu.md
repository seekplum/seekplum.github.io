...
```bash
for ip in $(pping -t 4); do hostname=$(sshpass -p xxx ssh -o ConnectTimeout=1 -o "StrictHostKeyChecking no" -i ~/.ssh/seekplum root@${ip} hostname 2>/dev/null); if [ $? = 0 ]; then echo "${hostname} ${ip}"; fi; done
```
